...
class Printer:
    ...
    def print_text(self):
        ...
        for dict_cert in self.array_dict_cert:
            print('Issued to: {}'.format(dict_cert["common_name_subject"]))
            print('Issued by: {}, {}'.format(dict_cert["common_name_issuer"], dict_cert["organization_name_issuer"]))
            print('Alternative names: {}'.format(",".join(str(name) for name in dict_cert["dns_names"])))
            ...
            if not dict_cert["has_expired"]:
                print('{} days to expire'.format(dict_cert["days_to_expire"]))
            else:
